...
struct EllipticalOAS: View {
    ...
    var body: some View {
        ...
        GeometryReader { geometry in
            ZStack {
                if( option < 4 ) { GrayTriangles() }
                Ellipses()
                EllipticalOAS_Live()
                CenterEllipse()
                if( option < 2 ) { EllipticalNoteNames() }
            }
        }
    }
}
        ...
        GeometryReader { geometry in
            ...
            ForEach( 0 ..< octaveCount, id: \.self) { oct in      //  0 <= oct < 7
                ...
                .foregroundStyle(
                    ...
                    (settings.option==0 || settings.option==4 ) ?
