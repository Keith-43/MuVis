...
// option0  grayTriangles    NoteNames   orange
// option1  grayTriangles    NoteNames   huGradient
// option2  grayTriangles                pomegranate
// option3  grayTriangles                teal
// option4  plain                        orange
// option5  plain                        hueGradient
// option6  plain                        pomegranate
// option7  plain                        teal
//
...
struct SpiralOAS: View {
    ...
    var body: some View {
        ...
        ZStack {
            if(option < 4) { SpiralOAS_LayoutBackground() }
            SpiralOAS_Live()
            if(option < 2) { EllipticalNoteNames() } // struct code in VisUtilities file
        }
    }
}
