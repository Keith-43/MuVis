/*
 ...
 Abstract:
 SpectralEnhancer is a normalization method used to enhance the display of
 spectral lines _(usually the harmonics of musical notes)_ and to reduce the display
 of noise (usually percussive effects which smear spectral energy over a large range.)
*/

import Accelerate

/**
 SpectralEnhancer is a normalization method used to enhance the display of spectral lines _(usually the harmonics of musical notes)_ and to reduce the display
 of noise (usually percussive effects which smear spectral energy over a large range.)
 
 The technique used here is to
 1. slide a moving window across  the spectrum seeking local peaks above some threshold.
 2. replace those peaks with the local mean value .
 3. compute a moving average of the resultant reduced-peaks spectrum.
 4. subtract this moving average from the initial spectrum.
 
 This technique is called "**Two-Pass Split-Window _(TPSW)_ Filtering**" and is described in the paper "Evaluation of Threshold-Based Algorithms for Detection of Spectral Peaks in Audio" by Leonardo Nunes, Paulo Esquef, and Luiz Biscainho.
...
class SpectralEnhancer {
    ...
    /// number of bins to include in average
    let filterWidth: Int
    let halfFilterWidth: Int
    /// A Hamming FFT window will smear the main peak over about 5 FFT bins.
    let gapWidth: Int
    let halfGapWidth: Int
    /// Determines which peaks are replaced by the mean during the first pass
    let noiseThreshold: Float
    
    init() {
        filterWidth = 65
        halfFilterWidth = Int(Double(filterWidth - 1) / 2)  // halfFilterWidth = 32
        gapWidth = 5
        halfGapWidth = Int(Double(gapWidth - 1) / 2)      // halfGapWidth = 2
        noiseThreshold = 2
    }
    ...
    // MARK: - localSum()
    /// This method returns the sliding-window sum of a one-dimensional array.
    func localSum(inputArray: [Float], windowLength: Int) -> [Float] {
    ...
    // MARK: - findMean()
    // TODO: run async
    /// This method generates an array that is the same as the inputArray but is free of prominent peaks.
    public func findMean(inputArray: [Float]) -> [Float] {
        ...
        let zeroesArray1 = [Float] (repeating: 0, count: halfFilterWidth )
        ...
        let zeroesArray2  = [Float](repeating: 0, count: halfGapWidth)
        ...
        var mean: Float = .zero
        var noPeaksArray = [Float](repeating: 0, count: inputArray.count)
        ...
        var binsInWindow =  [Float](repeating: Float(filterWidth), count: inputArray.count)
        ...
        var meanArray = [Float](repeating: 0, count: inputArray.count)
    ...
    //  MARK: - enhance()
    /// subtracts the meanArray from the original inputArray.
    public func enhance(inputArray: [Float]) -> [Float] {
        ...
        var outputArray = [Float](repeating: 0, count: inputArray.count)

        for i in 0 ..< inputArray.count {
            outputArray[i] = inputArray[i] - meanArray[i]
            if outputArray[i] < .zero { outputArray[i] = .zero }
        }
    ...
    public func pickPeaks(inputArray: [Float], peakThreshold: Float) -> [Bool] {
        ...
        var outputArray = [Bool](repeating: false, count: inputArray.count)
