...
//
///         Left      Left Note         Right    Right Note
///         Note       Freq              Note        Freq
///  Row 7:      C8  ( 4186.01 Hz)   to  B8   ( 7902.14 Hz)
///  Row 6:      C7  ( 2093.00 Hz)   to  B7   ( 3951.07 Hz)
///  Row 5:      C6  ( 1046.50 Hz)   to  B6   ( 1975.53 Hz)
///  Row 4:      C5  (  523.25 Hz)   to   B5   (  987.77 Hz)
///  Row 3:      C4  (  261.63 Hz)   to   B4   (  493.88 Hz)
///  Row 2:      C3  (  130.81 Hz)   to   B3   (  246.94 Hz)
///  Row 1:      C2  (   65.41 Hz)   to    B2   (  123.47 Hz)
///  Row 0:      C1  (   32.70 Hz)   to    B1   (   61.74 Hz)
///
///  Note Names:
///  Row 7:     C8   C#8  D8   D#8   E8   F8   F#8   G8   G#8   A8   A#8   B8
///  Row 6:     C7   C#7  D7   D#7   E7   F7   F#7   G7   G#7   A7   A#7   B7
///  Row 5:     C6   C#6  D6   D#6   E6   F6   F#6   G6   G#6   A6   A#6   B6
///  Row 4:     C5   C#5  D5   D#5   E5   F5   F#5   G5   G#5   A5   A#5   B5
///  Row 3:     C4   C#4  D4   D#4   E4   F4   F#4   G4   G#4   A4   A#4   B4
///  Row 2:     C3   C#3  D3   D#3   E3   F3   F#3   G3   G#3   A3   A#3   B3
///  Row 1:     C2   C#2  D2   D#2   E2   F2   F#2   G2   G#2   A2   A#2   B2
///  Row 0:     C1   C#1  D1   D#1   E1   F1   F#1   G1   G#1   A1   A#1   B1
///
///         leftFreq      bottomBin  binsInRow topBin    rightFreq
///  Row 7       4066.84         1511       1511        3021       8133.68
///  Row 6       2033.42           756        755         1510       4066.84
///  Row 5       1016.71           378        378           755       2033.42
///  Row 4         508.36           189        189           377       1016.71
///  Row 3         254.18             95          94           188         508.36
///  Row 2         127.09             48          47             94         254.18
///  Row 1           63.54             24          24             47         127.09
///  Row 0           31.77             12          12             23           63.54
///
...
///
///  In the AudioManager class, we specify that the audio sampleRate is 44,100 samples-per-second, and that the fftLength is 16,384 samples.
///  Hence, the spectrum contains 8,192 frequency "bins" - separated by binFreqWidth = (44100 / 2) / 8192 = 2.69165 Hz
///
///  This NoteProcessing class calculates various parameters relating the frequency bins provided by the AudioManager's FFT function
...
class NoteProcessing {
    ...
    var lowerOctFreq = [Double](repeating: 0.0, count: 8)           // frequency at the lower border for a given octave
    var upperOctFreq = [Double](repeating: 0.0, count: 8)           // frequency at the upper border for a given octave

    var lowerNoteFreq = [Double](repeating: 0.0, count: eightOctNoteCount+1) // frequency at the lower border of a each note
    var upperNoteFreq = [Double](repeating: 0.0, count: eightOctNoteCount+1) // frequency at the upper border of a each note
    ...
    func calculateParameters() {
        ...
        freqB8  = 7040.0 * pow(twelfthRoot2, 2.0)   // B8 = 7,902.134 Hz        B8 is 2 semitones above A8=7,040 Hz
    ...
    public func computeHarmSumSpectrum(inputArray: [Float]) -> [Float] {
        // The inputArray is typically a muSpectrum from the AudioManager and has length eightOctPointCount.
        // The outputArray is the "Harmonic Sum Spectrum" and has length sixOctPointCount.
        var harmSumSpectrum = [Float] (repeating: 0.0, count: sixOctPointCount) // sixOctPointCount = 72*12 = 864
